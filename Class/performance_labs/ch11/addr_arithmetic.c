...
char * split_the_string(char * string_ptr, char delimiter)
{
	//Holds the post delimiter half of split string.
    char *holderPtr = NULL;
	...
	//Loops for the length of the original string.
    for(int i = 0; i < strlen(string_ptr); i++)
    {
		//Runs if the current spot of the string is the delimiter character.
        if(*(string_ptr + i) == delimiter)
        {
			//Stores the first spot post delimiter if the holderPtr is still NULL
            if(holderPtr == NULL)
            {
                holderPtr = (string_ptr + i + 1);  //Save the next position. Because the Delimeter will be deleted on Success                
            }
			//Return error if holderPtr != NULL
            else
            {
	...
	//Return the original string if nothing was stored into the holderPtr.
    if(holderPtr == NULL)
    {
	    return string_ptr;        
    }
	//Turns the delimiter into a null character and return the split string.
    else
    {
        *(holderPtr - 1) = '\0';
